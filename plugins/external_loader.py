...
try:
    ...
    shm.buf[0] = 10
    shm.close()
    # logger.error(f"The plugin '{data['name']}' has been started in a venv without Pyro5!")
    raise e
...
if not to_load["DISABLE_PLUGIN_MODULE_CHECK"]:
    for i in to_load["missing_imports"]:
        try:
            ...
            print(f"INFO: The loading of '{to_load['name']}' has been stopped. The server assumes that the venv you are in"
                  f" does not fulfill the requirements to load the plugin.\nThe suspected missing module is '{i}'."
...
# sys.path.insert(0, os.path.join(to_load["server_path"], "plugins") )
# above_server = os.path.dirname(to_load["server_path"])
# sys.path.insert(0, above_server)
