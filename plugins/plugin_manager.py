...
class _PluginLoader:
    ...
    def __init__(self, jupyter=False, as_server=True):
        if as_server:
        ...
        self.python_version = _platform.python_version_tuple()
        if self.python_version[0] != '3':
            _logger.error("Python 2 is not supported!")
            _sys.exit()
        if int(self.python_version[1]) < 11:
            _logger.warning("Python version lower than recommended (> 3.11.X).")
        if int(self.python_version[1]) < 4:
            _logger.error("Python versions < 3.4 will not work!")
        if _sys.base_prefix == _sys.prefix:
            _logger.warning("System python interpreter in use. This is discouraged!")
    ...
    def register_consumer(channel_name, scope):
        ...
        new_api = api.API(channel_name, scope)
    ...
    def finish_loading(self):
        ...
        _logger.debug(f"Loaded plugins: \"{list(_PluginLoader.loaded_plugins.keys())}\". All registered functions: "
                      f"\"{list(api._available_functions.keys())}\"")
            ...
            if not conf_dic["is_local"]:
                ...
                if conf_dic["venv_path"]:
                    ...
                    if result.returncode == 0:
                        ...
                        # An error occurred, you can access the error message as a string
                    ...
                    if len(missing_imports) > 0 and not settings.DISABLE_PLUGIN_MODULE_CHECK:
                        _logger.critical(f"The server thinks the venv which you specified for '{plugin_config['name']}'"
                                         f" does not contain all packages required for running the plugin. Likely "
